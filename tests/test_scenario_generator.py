...
class TestScenarioGenerator:
    ...
    def test_generate_transaction_scenarios(self):
        ...
        scenarios = generator.generate_scenarios(
            components, "Chatbot for transaction disputes"
        )
    ...
    def test_generate_prompt_injection_scenarios(self):
        ...
        scenarios = generator.generate_prompt_injection_scenarios(
            components, "Add chatbot"
        )

        # Should generate prompt injection scenarios for AI components
        assert len(scenarios) > 0
        assert any(
            "ignore previous instructions" in scenario.scenario.lower()
