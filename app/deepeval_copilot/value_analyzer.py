...
class ValueAnalyzer:
    ...
    def analyze_evaluation_value(
        ...
        quick_result = self._quick_value_check(issue_title, issue_body, issue_type, components)
    ...
    def _quick_value_check(
        ...
        non_valuable_keywords = [
            "documentation", "readme", "typo", "spelling", "formatting",
            "comment", "refactor only", "cleanup", "style", "lint"
        ]
        
        if any(keyword in text for keyword in non_valuable_keywords):
            return False, f"Issue appears to be {non_valuable_keywords[0]}-related, unlikely to provide evaluation value"
        ...
        if not components.conversational_ai and not components.backend_api:
            return False, "Issue does not involve AI or backend components, unlikely to provide evaluation value"
        ...
        if issue_type in ["Bug", "Tech Debt"] and components.conversational_ai:
            return True, f"{issue_type} involving AI components typically provides evaluation value"
        ...
        if "provides value: no" in response_lower:
            ...
            for line in response.split("\n"):
                if "reasoning:" in line.lower():
                    ...
                    break
            return False, reasoning or "AI determined issue does not provide evaluation value"
