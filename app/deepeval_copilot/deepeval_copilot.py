...
from deepeval_copilot.models import ComponentDetection, DeepEvalStrategy
...
class DeepEvalCopilot:
    ...
    def analyze_issue(
        ...
        if not provides_value:
            return DeepEvalStrategy(
                ...
                quality_risks=[],
                recommendations=[
                    f"This issue does not provide evaluation value: {value_assessment}"
                ],
        ...
        conversational_test_cases = (
            self.scenario_generator.generate_conversational_test_cases(
        ...
        prompt_injection_tests = (
            self.scenario_generator.generate_prompt_injection_tests(
                components, title, body
            )
        )
    ...
    def _generate_recommendations(
        ...
        if metric_recommendations:
            metric_names = [
                metric.name for metric in metric_recommendations if metric.applies
            ]
            recommendations.append(
...
Format each recommendation on a new line, starting with "- "."""
    ...
    def format_strategy_as_comment(self, strategy: DeepEvalStrategy) -> str:
        ...
        if strategy.components.frontend:
            lines.append(
                "✓ **Frontend UI** (chatbot interface, user interaction elements, client-side display)"
            )
        if strategy.components.backend_api:
            lines.append(
                "✓ **Backend API** (data processing, service endpoints, system integration)"
            )
        if strategy.components.conversational_ai:
            lines.append(
                "✓ **Conversational AI** (chatbot logic, NLP, AI response generation)"
            )
        lines.append("")
        ...
        if strategy.evaluation_data:
            ...
            for data in strategy.evaluation_data:
                ...
                lines.append(
                    f'"{question}","{ground_truth}","{data.metric_name}","{business_context}"'
                )
            lines.append("```")
            lines.append("")
        ...
        if strategy.conversational_test_cases:
            ...
            for test in strategy.conversational_test_cases:
                ...
                if test.context_requirements:
                    lines.append(
                        f"**Context Requirements:** {', '.join(test.context_requirements)}"
                    )
                ...
                for turn in test.turns:
                    ...
                    lines.append(
                        f"🎯 **Ground Truth:** {', '.join(turn.ground_truth_requirements)}"
                    )
                    lines.append(
                        f"📊 **Metrics:** {', '.join(turn.evaluation_metrics)}"
                    )
                    lines.append("")
                lines.append("")
        ...
        if strategy.prompt_injection_tests:
            ...
            for test in strategy.prompt_injection_tests:
                ...
                lines.append(
                    f"**Evaluation Metrics:** {', '.join(test.evaluation_metrics)}"
                )
        ...
        if strategy.conversational_test_cases:
            ...
            lines.append("```csv")
            lines.append(
                "test_id,turn_id,user_input,expected_behavior,evaluation_metrics,ground_truth_requirements"
            )
            ...
            for test in strategy.conversational_test_cases:
                for turn in test.turns:
                    ...
                    lines.append(
                        f"{test.test_id},{turn.turn_id},{user_input},{expected},{metrics},{requirements}"
                    )
