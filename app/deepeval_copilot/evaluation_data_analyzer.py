...
class EvaluationDataAnalyzer:
    ...
    def suggest_evaluation_data(
        ...
        logger.info(f"Suggested {len(evaluation_data)} evaluation data entries for issue: {issue_title}")
        return evaluation_data
    ...
    def _build_evaluation_data_prompt(
        ...
        context_parts.append(f"Applicable Metrics:\n" + "\n".join(metric_info))
...
- Are practical and realistic for the business context"""
    ...
    def _create_evaluation_data(self, data: dict, applicable_metrics: List[MetricRecommendation]) -> EvaluationData:
